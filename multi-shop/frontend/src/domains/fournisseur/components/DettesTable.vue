<template>
  ...
  >
    ...
    >
      ...
      <Column v-if="showFournisseurColumn && !isMobile" header="Fournisseur">
        <template #body="{ data }">{{ data.fournisseur_name || '—' }}</template>
      </Column>
      <Column header="Référence">
        <template #body="{ data }">{{ data.reference }}</template>
      </Column>
      <Column v-if="!isMobile" header="Commande">
        <template #body="{ data }">{{ data.commande_reference || '—' }}</template>
      </Column>
      ...
      <Column header="Actions" style="width: 90px">
        <template #body="{ data }">
          <AppTableActionsMenu
            :actions="detteRowActions(data)"
            aria-label="Actions dette"
          />
        </template>
      </Column>

      <template #expansion="{ data }">
        <div class="dettes-table__expansion">
          <h4 class="dettes-table__expansion-title">Paiements</h4>
          <DataTable
          ...
          <p v-else class="dettes-table__empty">Aucun paiement enregistré.</p>
        </div>
      </template>
    </DataTable>
  </AppTableState>
...
const props = defineProps({
  ...
  emptyText: {
    type: String,
    default: 'Aucune dette ne correspond aux filtres sélectionnés.'
  }
})
...
const canPay = (dette) => dette.statut === 'en_cours' && Number(dette.balance) > 0

const detteRowActions = (dette) => [
  {
    label: 'Décaisser',
    icon: 'pi pi-wallet',
    severity: 'warn',
    visible: canPay(dette),
    loading: props.paymentLoadingId === dette.id,
    command: () => emit('pay', dette)
  }
]
...
const filteredItems = computed(() => {
  ...
  return props.items.filter((item) => {
    ...
    return [
      item.fournisseur_name,
      item.reference,
      item.commande_reference,
      item.label
...
<style scoped>
.dettes-table__expansion {
  padding: 0.5rem 0.75rem 0.75rem;
}

.dettes-table__expansion-title {
  margin: 0 0 0.75rem;
  font-size: 0.95rem;
}
