...
const loadHistory = async () => {
  ...
  try {
    ...
    error.value = err?.response?.data?.error || err?.message || 'Impossible de charger l\'historique de connexion.'
...
<template>
  <div class="login-history-panel">
    ...
    <Message v-if="error" severity="error" class="login-history-panel__error">
      {{ error }}
      <Button label="Réessayer" icon="pi pi-refresh" text @click="loadHistory" />
    </Message>

    <Message v-else-if="!loading && items.length === 0" severity="info">
