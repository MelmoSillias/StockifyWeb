...
#[AsCommand(
    name: 'access:sync-analytics-permissions',
    description: 'Ajoute les permissions analytics.* manquantes et les lie aux rôles',
)]
final class SyncAnalyticsPermissionsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $analyticsDefs = array_values(array_filter(
            PermissionCatalog::all(),
            static fn (array $def): bool => str_starts_with($def['code'], 'analytics.'),
        ));

        $permissionsByCode = [];
        $created = 0;

        foreach ($analyticsDefs as $def) {
        ...
        foreach (PermissionCatalog::rolePermissions() as $roleCode => $permissionCodes) {
            ...
            foreach ($permissionCodes as $code) {
                if (!str_starts_with($code, 'analytics.') || !isset($permissionsByCode[$code])) {
                    continue;
                }
