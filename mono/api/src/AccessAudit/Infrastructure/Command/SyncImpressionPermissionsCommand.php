...
#[AsCommand(
    name: 'access:sync-impression-permissions',
    description: 'Ajoute les permissions impression.* manquantes et les lie aux rôles',
)]
final class SyncImpressionPermissionsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $impressionDefs = array_values(array_filter(
            PermissionCatalog::all(),
            static fn (array $def): bool => str_starts_with($def['code'], 'impression.'),
        ));

        $permissionsByCode = [];
        $created = 0;

        foreach ($impressionDefs as $def) {
        ...
        foreach (PermissionCatalog::rolePermissions() as $roleCode => $permissionCodes) {
            ...
            foreach ($permissionCodes as $code) {
                if (!str_starts_with($code, 'impression.') || !isset($permissionsByCode[$code])) {
                    continue;
                }
