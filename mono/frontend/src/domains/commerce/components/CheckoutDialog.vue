<template>
  ...
  >
    <div class="checkout-dialog">
      ...
      <div v-if="isQuoteMode" class="checkout-dialog__field">
        <label for="checkout-valid-until">Valable jusqu'au</label>
        <DatePicker
          id="checkout-valid-until"
          v-model="validUntil"
          date-format="dd/mm/yy"
          show-icon
          :min-date="operationDate"
          fluid
        />
        <small class="checkout-dialog__hint">Date limite de validité du devis.</small>
      </div>
...
const onConfirm = () => {
  ...
  if (isQuoteMode.value && !validUntil.value) {
    return
  }
