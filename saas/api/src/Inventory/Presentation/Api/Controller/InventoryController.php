...
final class InventoryController extends AbstractController
{
    ...
    #[IsGranted(ShopPermissionVoter::MANAGE_STOCK)]
    ...
    public function listMovements(Request $request): JsonResponse
    {
        ...
        $variantUuid = $variantId && Uuid::isValid($variantId) ? Uuid::fromString($variantId) : null;
        $movements = $this->stockMovementRepository->findByShop($shopId, $variantUuid);

        return $this->json(array_map([$this, 'serializeMovement'], $movements));
    }
    ...
    {
        $alerts = $this->stockAlertsHandler->handle(
            GetStockAlertsQuery::forCurrentShop($this->tenantContextHolder),
        );
    ...
    private function getVariant(string $id): \App\Catalog\Domain\Entity\ProductVariant
    {
        $variant = $this->variantRepository->findForShop(
            $this->tenantContextHolder->get()->getShop()->getId(),
            Uuid::fromString($id),
        );
        if (null === $variant) {
    ...
    private function serializeLot(\App\Inventory\Domain\Entity\StockLot $lot): array
    {
        return [
            ...
            'reference' => $lot->getVariant()->getSku(),
